...
it all together, I run the program as follows:
...
Safepoint poll. The VM needs threads to occasinally poll for safe points. At a safe point, the JVM state for the current
...
which tells us in which registers the arguments are being passed. The HotSpot JITs use a custom calling convetion for
